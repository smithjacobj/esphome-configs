...
CONF_AUDIO_EQ = "tone"
CONF_AUDIO_EQ_LO = "lo"
CONF_AUDIO_EQ_MID = "mid"
CONF_AUDIO_EQ_HI = "hi"

AUDIO_EQ_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.Optional(CONF_AUDIO_EQ_LO, default=0): cv.float_range(-40, 6),
            cv.Optional(CONF_AUDIO_EQ_MID, default=0): cv.float_range(-40, 6),
            cv.Optional(CONF_AUDIO_EQ_HI, default=0): cv.float_range(-40, 6),
        }
    )
)
            ...
            "external": media_player.MEDIA_PLAYER_SCHEMA.extend(
                {
                    ...
                    cv.Optional(CONF_AUDIO_EQ): AUDIO_EQ_SCHEMA,
                }
            ).extend(cv.COMPONENT_SCHEMA),
        },
        key=CONF_DAC_TYPE,
...
async def to_code(config):
    ...
    if config[CONF_AUDIO_EQ] is not None:
        cg.add(var.set_eq_lo(config[CONF_AUDIO_EQ][CONF_AUDIO_EQ_LO]))
        cg.add(var.set_eq_mid(config[CONF_AUDIO_EQ][CONF_AUDIO_EQ_MID]))
        cg.add(var.set_eq_hi(config[CONF_AUDIO_EQ][CONF_AUDIO_EQ_HI]))
