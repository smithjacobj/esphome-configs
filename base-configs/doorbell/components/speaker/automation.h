...
template<typename... Ts> class PlayAction : public Action<Ts...>, public Parented<Speaker> {
 public:
  void set_data_template(std::function<const std::span<const uint8_t>(Ts...)> func) {
    this->data_func_ = func;
    this->static_ = false;
  }
  void set_data_static(const std::span<const uint8_t> &data) {
 ...
 protected:
  bool static_{false};
  std::function<const std::span<const uint8_t>(Ts...)> data_func_{};
  std::span<const uint8_t> data_static_{};
};
